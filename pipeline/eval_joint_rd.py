...
# ---------- Contextual entropy model (same as training) ----------
...
# Load joint-trained models for each λ
lambda_list = [0.5, 1.0, 5.0]
results = []
for lam in lambda_list:
    ckpt_dir = f'/root/le-maia/checkpoints_joint_rd'
    ae_ckpt = f'{ckpt_dir}/ae_lambda_{lam}_final.pt'
    entropy_ckpt = f'{ckpt_dir}/entropy_lambda_{lam}_final.pt'
    
    if not os.path.exists(ae_ckpt) or not os.path.exists(entropy_ckpt):
    ...
    with torch.no_grad():
        for frame in frames:
            ...
            # Add time dimension
            recon, latent_norm = autoencoder(frame_t.unsqueeze(0))
            ...
            total_mse += mse
    avg_bpp = total_bits / (len(frames) * 256 * 256)
...
with open('/root/le-maia/benchmark_results/joint_rd.csv', 'w') as f:
    writer = csv.writer(f)
...
print("Results saved to /root/le-maia/benchmark_results/joint_rd.csv")
