#!/usr/bin/env python3
"""
Evaluate all λ checkpoints from joint training.
Compute bpp, PSNR, and compare with x265.
"""
...
def evaluate_lewm(ae_ckpt, ent_ckpt):
    ...
    total_bits = 0
    total_mse = 0
    for frame in tqdm(frames, desc=f"Evaluating {os.path.basename(ae_ckpt)}"):
...
for lam in lambda_list:
    ...
    bpp, psnr = evaluate_lewm(ae_ckpt, ent_ckpt)
...
csv_path = os.path.join(BENCHMARK_DIR, 'rd_curve_final.csv')
