...
class LeWMEncoder(nn.Module):
    ...
    Architecture:
        - Patchify layer: 32x32 patches, projects to hidden_dim=192
        - 6-layer Transformer encoder (Tiny variant)
        - Final projection to latent_dim=32
        - Optional semantic surprise branch (MLP head)

    Args:
        latent_dim: Output latent dimension (default: 32)
        patch_size: Size of patches for patchification (default: 32)
    ...
    Output:
        latent: Latent tensor of shape [B, latent_dim, H//32, W//32]
