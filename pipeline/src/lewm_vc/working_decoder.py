...
class LeWMDecoder(nn.Module):
    ...
    Video decoder with improved architecture:
        ...
        - 5 upsampling blocks with residual connections (8×8 grid → 256×256)
        - Deeper processing for better reconstruction

    Args:
        latent_dim: Input latent dimension (default: 32)
    ...
    def __init__(
        ...
        self.up1 = nn.ConvTranspose2d(hidden_dim, hidden_dim, kernel_size=4, stride=2, padding=1)
        ...
        self.up3 = nn.ConvTranspose2d(hidden_dim, hidden_dim, kernel_size=4, stride=2, padding=1)
        self.res3 = ResidualBlock(hidden_dim)

        self.up4 = nn.ConvTranspose2d(hidden_dim, hidden_dim // 2, kernel_size=4, stride=2, padding=1)
        self.res4 = ResidualBlock(hidden_dim // 2)

        self.up5 = nn.ConvTranspose2d(hidden_dim // 2, hidden_dim // 4, kernel_size=4, stride=2, padding=1)
        ...
        self.final = nn.Sequential(
            ...
            nn.GELU(),
            nn.Conv2d(hidden_dim // 8, output_channels, 3, padding=1),
        )
