#!/usr/bin/env python3
"""
Evaluate all JEPA λ checkpoints + compare with x265.
Encodes using predictor-based residual coding: I-frame (intra) + P-frames (residual).

Usage:
    python3 jepa_eval.py
"""
...
# ---------- Evaluate JEPA ----------
def evaluate_jepa(ae_ckpt, pred_ckpt, ent_ckpt):
    ...
    for frame in tqdm(frames, desc=f"JEPA {os.path.basename(ae_ckpt)}"):
...
for lam in lambda_list:
    ...
    pred_ckpt = os.path.join(CHECKPOINT_DIR, f'predictor_lambda_{lam}_final.pt')
    ...
    if not os.path.exists(ae_ckpt) or not os.path.exists(pred_ckpt) or not os.path.exists(ent_ckpt):
        ...
        continue
    print(f"Evaluating JEPA λ={lam}...")
    bpp, psnr = evaluate_jepa(ae_ckpt, pred_ckpt, ent_ckpt)
...
csv_path = os.path.join(BENCHMARK_DIR, 'rd_curve_jepa.csv')
