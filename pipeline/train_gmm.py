#!/usr/bin/env python3
"""
Entropy model with Mixture of Gaussians (GMM) – 2 components.
Trains from scratch on PEViD-HD.
"""
...
class GMMEntropyModel(nn.Module):
    ...
    def forward(self, x):
        ...
        mu = params[:, :, :self.latent_dim, :, :]          # [B, num_comp, latent_dim, H, W]
        ...
        weight = torch.softmax(log_weight, dim=1)          # normalise over components
        return mu, scale, weight

def gmm_likelihood_discrete(y, mu, scale, weight, step, epsilon=1e-9):
    """
    Compute negative log-likelihood of quantized y under GMM.
    y: [B, latent_dim, H, W]
    mu, scale, weight: [B, num_comp, latent_dim, H, W]
    """
    B, C, H, W = y.shape
    num_comp = mu.shape[1]
    # Expand y to [B, num_comp, C, H, W]
    y_expanded = y.unsqueeze(1).expand(-1, num_comp, -1, -1, -1)
    # Compute CDF of each Gaussian
...
use_perceptual = False  # Disable LPIPS to avoid warnings and speed up
...
output_dir = '/root/le-maia/checkpoints_gmm'
...
for LAMBDA in lambda_list:
    ...
    if os.path.exists(ae_phase0):
        autoencoder.load_state_dict(torch.load(ae_phase0, map_location=device, weights_only=False), strict=False)
        print("Loaded Phase 0 autoencoder")
    autoencoder.train()

    entropy_model = GMMEntropyModel(latent_dim=192, hyper_channels=256, num_components=2).to(device)
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        total_mse = 0
        num_batches = 0
        pbar = tqdm(dataloader, desc=f"GMM λ={LAMBDA} Epoch {epoch}/{EPOCHS}")
        for batch in pbar:
...
print("GMM per-λ training complete.")
