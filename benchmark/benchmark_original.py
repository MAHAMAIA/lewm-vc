...
autoencoder.eval()
print("Original autoencoder loaded.")

# Load NLL entropy model
...
def encode_video(autoencoder, entropy_model, quantizer, frames):
    T = len(frames)
    ...
    for t in range(T):
        ...
        with torch.no_grad():
            ...
            total_bits += bits
            recon = autoencoder.decoder(quantized, target_size=(h,w))
...
print("Encoding with LeWM-VC (original autoencoder + entropy model)...")
