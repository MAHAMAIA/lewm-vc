#!/usr/bin/env python3
"""
Validate Phase 1 models with real bitstream writer and predictor.
Computes BD-rate and generates wow demo.
"""
...
# Load trained predictor (from earlier)
...
if os.path.exists(predictor_path):
    ...
    print("Predictor not found; will use I-frames only.")
...
def encode_video_with_lambda(model, frames, gop_size=16):
    """Encode frames using a given λ model, write real bitstream, return decoded frames and total bits."""
    T = len(frames)
...
for lam in lambda_list:
    ...
    # Use the same predictor (trained) for all λ
    model = FullCodec(autoencoder, entropy_model, quantizer, predictor).to(device)
    state_dict = torch.load(ckpt_path, map_location=device)
    # The checkpoint contains the FullCodec state dict (with keys 'autoencoder', 'entropy_model', 'quantizer')
    # We only need entropy_model and quantizer (autoencoder is already loaded)
    ...
    # If the checkpoint is a flat state dict (like from train_phase1.py), it might have keys like 'autoencoder.encoder...'
    # In that case we can load directly into model.
    # For safety, try loading the entire model.
    try:
        model.load_state_dict(state_dict, strict=False)
    except:
        print("Warning: Could not load state dict directly; skipping λ={lam}")
        continue
    model.eval()
...
csv_path = '/root/le-maia/benchmark_results/phase1_bdrate_fixed.csv'
...
# ------------------------------
# Generate demo for best λ (highest PSNR)
# ------------------------------
if lewm_results:
    best_lam = lewm_results[np.argmax([r[1] for r in lewm_results])][2]
    print(f"\nGenerating demo using λ={best_lam}...")
    # Reload best model
    entropy_model = HyperpriorEntropy(latent_dim=192, hyper_channels=320).to(device)
    quantizer = Quantizer(num_levels=256, mode='inference').to(device)
    model = FullCodec(autoencoder, entropy_model, quantizer, predictor).to(device)
    ckpt_path = f'/root/le-maia/checkpoints/phase1_lambda_{best_lam}/final.pt'
    state_dict = torch.load(ckpt_path, map_location=device)
    model.load_state_dict(state_dict, strict=False)
    model.eval()

    # Use anomaly clip (dropping bag)
    demo_video_path = '/root/le-maia/datasets/pevid-hd/droppingBag_day_indoor_1_1.mpg'
    ...
    cap.release()
    # Use segment with anomaly (frames 50-150)
    demo_frames = demo_frames[50:150]
    print(f"Demo: {len(demo_frames)} frames")

    # Encode with our model to get bitmaps and surprise (we need per-patch bits)
    # For simplicity, we reuse encode_video_with_lambda but also capture bits per frame.
    # We'll compute heatmaps from the quantized residual entropy.
    decoded_demo = []
    bit_maps = []
    writer_demo = BitstreamWriter(version=1)
    prev_quantized = None
    for t, frame in enumerate(tqdm(demo_frames, desc="Encoding demo")):
        frame_t = torch.from_numpy(frame).float().permute(2,0,1).unsqueeze(0) / 255.0
        frame_t = frame_t.to(device)
        with torch.no_grad():
            latent = model.autoencoder.encoder(frame_t, return_surprise=False)
            quantized = model.quantizer(latent)
            if t % 16 == 0 or prev_quantized is None:
                frame_data = {"latent": quantized.cpu()}
                nal_bytes = writer_demo.write_frame(frame_data, is_iframe=True)
                # For heatmap, use entropy of quantized latents
                rate_nats, _ = model.entropy_model(quantized)
                bits_per_patch = (rate_nats * np.log2(np.e)).cpu().numpy()[0,0]  # [H,W]
                bit_maps.append(bits_per_patch)
                prev_quantized = quantized
            else:
                pred_mean, _ = model.predictor([prev_quantized])
                residual = latent - pred_mean
                ...
                reconstructed_latent = pred_mean + quantized_residual
                frame_data = {"residual": quantized_residual.cpu()}
                nal_bytes = writer_demo.write_frame(frame_data, is_iframe=False)
                rate_nats, _ = model.entropy_model(quantized_residual)
                bits_per_patch = (rate_nats * np.log2(np.e)).cpu().numpy()[0,0]
                bit_maps.append(bits_per_patch)
                prev_quantized = reconstructed_latent
            recon = model.autoencoder.decoder(prev_quantized, target_size=(256,256))
            recon_np = (recon.squeeze(0).permute(1,2,0).cpu().numpy() * 255).astype(np.uint8)
            decoded_demo.append(recon_np)

    # Encode same segment with x265 at low bitrate (CRF 36)
    temp_input = '/tmp/demo_input.mp4'
    temp_output = '/tmp/demo_x265.mp4'
    h, w = demo_frames[0].shape[:2]
    out = cv2.VideoWriter(temp_input, cv2.VideoWriter_fourcc(*'mp4v'), 30, (w,h))
    for f in demo_frames:
        out.write(cv2.cvtColor(f, cv2.COLOR_RGB2BGR))
    out.release()
    subprocess.run(['ffmpeg', '-y', '-i', temp_input, '-c:v', 'libx265', '-crf', '36', '-preset', 'medium', temp_output], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    cap.release()
    # Trim to same length
    min_len = min(len(demo_frames), len(x265_frames), len(decoded_demo))
    demo_frames = demo_frames[:min_len]
    x265_frames = x265_frames[:min_len]
    decoded_demo = decoded_demo[:min_len]
    bit_maps = bit_maps[:min_len]
    ...
    out_video = cv2.VideoWriter('/root/le-maia/wow_demo.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 30, (w*2, h))
    for i in range(min_len):
        left = cv2.cvtColor(x265_frames[i], cv2.COLOR_RGB2BGR)
        right = cv2.cvtColor(decoded_demo[i], cv2.COLOR_RGB2BGR)
        right = overlay_heatmap(right, bit_maps[i], alpha=0.5, color_map=cv2.COLORMAP_HOT)
        cv2.putText(left, 'x265 (CRF 36)', (10,30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2)
        cv2.putText(right, f'LeWM-VC (λ={best_lam})', (10,30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2)
    ...
    out_video.release()
    print("Demo saved to /root/le-maia/wow_demo.mp4")
else:
    print("No LeWM-VC results, skipping demo.")
