#!/usr/bin/env python3
"""
Generate demo videos using the best joint-trained checkpoint (λ=0.05).
Left: x265 (CRF 36), Right: LeWM-VC with dynamic heatmap (bits per patch).
"""
...
if not os.path.exists(ae_ckpt) or not os.path.exists(ent_ckpt):
    raise FileNotFoundError(f"Best checkpoints for λ={LAMBDA} not found in {checkpoint_dir}")
