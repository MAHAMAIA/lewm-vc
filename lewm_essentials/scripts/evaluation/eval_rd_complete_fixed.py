...
FIXED: checkpoint paths, λ list, dtype casting, x265 preset, BPP calculation, BD-rate sorting.
"""
...
def compute_bd_rate(rate1, psnr1, rate2, psnr2):
    ...
    # Sort by PSNR (required for monotonic interpolation)
...
# ✅ FIXED: λ values to evaluate
lambda_list = [0.005, 0.01, 0.05, 0.1, 0.5, 1.0]
checkpoint_dir = '/root/le-maia/checkpoints_amd_final'
results = []
...
for lam in lambda_list:
    ...
    psnr_y = 10 * np.log10(1.0 / (total_mse_y / len(frames))) if total_mse_y > 0 else 100
...
def encode_x265(video_path, crf, target_size, num_frames=150):
    ...
    cmd = ['ffmpeg', '-y', '-i', video_path, '-vf', f'scale={target_size[0]}:{target_size[1]}', 
           '-c:v', 'libx265', '-crf', str(crf), '-preset', 'veryslow', out_path]  # ✅ FIXED: veryslow
...
# ✅ FIXED: Filter valid results and sort for BD-rate
...
bd_rate = None
if len(valid_results) >= 2 and len(valid_x265) >= 2:
    # Sort by PSNR for monotonic interpolation
    ...
    print(f"\nLeWM-VC valid points: {len(valid_results_sorted)}")
    print(f"x265 valid points: {len(valid_x265_sorted)}")
    print(f"PSNR overlap range: [{max(min(my_psnrs), min(x265_psnrs)):.2f}, {min(max(my_psnrs), max(x265_psnrs)):.2f}] dB")
    
    bd_rate = compute_bd_rate(my_rates, my_psnrs, x265_rates, x265_psnrs)
    if not np.isnan(bd_rate):
        print(f"\n✅ BD-rate vs x265: {bd_rate:+.2f}%")
    else:
        print("\n⚠️ BD-rate could not be computed (insufficient PSNR overlap)")
...
with open('/root/le-maia/benchmark_results/rd_curve.csv', 'w') as f:
    ...
    if bd_rate is not None and not np.isnan(bd_rate):
        ...
        writer.writerow([])
        writer.writerow(['BD-rate vs x265', 'N/A (insufficient PSNR overlap)'])

print("\n📁 Results saved to /root/le-maia/benchmark_results/rd_curve.csv")

# Print summary
print("\n" + "="*60)
print("LEWM-VC RD POINTS")
print("="*60)
for lam, bpp, psnr in sorted(results, key=lambda x: x[2]):
    print(f"λ={lam:5} | BPP={bpp:.4f} | Y-PSNR={psnr:.2f} dB")

print("\n" + "="*60)
print("X265 BASELINE")
print("="*60)
for crf, bpp, psnr in sorted(x265_results, key=lambda x: x[2]):
    print(f"CRF={crf:3} | BPP={bpp:.4f} | Y-PSNR={psnr:.2f} dB")
