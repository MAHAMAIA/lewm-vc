...
# ---------- Affine autoencoder definition (same as training) ----------
...
with torch.no_grad():
    ...
    print(f"Quantized: unique values={torch.unique(quantized).numel()}, min={quantized.min().item():.6f}, max={quantized.max().item():.6f}")
    ...
    # Also test with a random latent to ensure entropy model works
    random_latent = torch.randn(1, 192, 16, 16).to(device) * 0.45 + 0.05
    random_quantized = quantizer(random_latent)
    random_rate_nats, _ = entropy_model(random_quantized)
    random_bits = random_rate_nats.sum().item() * 1.4427
    print(f"Random latent rate bits: {random_bits:.4f}")
