...
Measures surprise as MSE between predicted and actual latent.
Compares normal vs. anomaly frames.
"""
...
# ---------- Affine autoencoder (same as training) ----------
...
if os.path.exists(predictor_ckpt):
    predictor.load_state_dict(torch.load(predictor_ckpt, map_location=device, weights_only=False))
    predictor.eval()
    print("Predictor loaded.")
else:
    print("Predictor checkpoint not found. Using fallback: prediction = zero (surprise = latent norm).")
    predictor = None
...
for i, frame in enumerate(tqdm(frames, desc="Computing surprise")):
    ...
    with torch.no_grad():
        latent = autoencoder.encode(frame_t)  # [1,192,8,8] for 128x128 (latent 8x8)
        
        if predictor is not None and len(context) >= 1:
            # Use context of previous latents (up to 4)
            pred_mean, _ = predictor(context[-4:])
            surprise = torch.nn.functional.mse_loss(pred_mean, latent).item()
        else:
            # Fallback: surprise = energy of latent (norm)
            surprise = latent.norm().item()
        
        surprise_scores.append(surprise)
        context.append(latent)
        # Keep context length limited
        if len(context) > 4:
            context.pop(0)
...
with open('/root/le-maia/benchmark_results/voe_predictor_results.txt', 'w') as f:
...
print("Results saved to /root/le-maia/benchmark_results/voe_predictor_results.txt")
