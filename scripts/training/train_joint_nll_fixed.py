...
def nll_loss(mu, sigma, x):
    """Mean negative log-likelihood per element."""
    var = sigma ** 2
    loss = 0.5 * ((x - mu) ** 2 / var + torch.log(var) + np.log(2 * np.pi))
    return loss.mean()  # average over all elements

LAMBDA = 0.01  # start small
EPOCHS = 100
os.makedirs('/root/le-maia/checkpoints_joint_nll_fixed', exist_ok=True)
...
for epoch in range(1, EPOCHS+1):
    ...
    for batch in pbar:
        ...
        # Rate: mean NLL per element * number of elements * bits per nat
        nll = nll_loss(mu, sigma, quantized)
        num_elements = quantized.numel()
        rate_bits = nll * num_elements * np.log2(np.e)
    ...
    if epoch % 20 == 0:
        ...
        }, f'/root/le-maia/checkpoints_joint_nll_fixed/checkpoint_epoch{epoch}.pt')

torch.save(autoencoder.state_dict(), '/root/le-maia/checkpoints_joint_nll_fixed/autoencoder_final.pt')
torch.save(entropy_model.state_dict(), '/root/le-maia/checkpoints_joint_nll_fixed/entropy_final.pt')
