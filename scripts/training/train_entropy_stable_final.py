#!/usr/bin/env python3
"""
Stable entropy model training for LeWM-VC.
Uses discrete Gaussian likelihood with proper quantizer step,
sigma floor, optional latent scaling, and diagnostic logging.
"""
...
# ---------- Quantizer diagnostic and step size ----------
...
with torch.no_grad():
    ...
    quantizer_test = Quantizer(num_levels=256, mode='inference').to(device)
    quantized_test = quantizer_test(latent_norm)
    print(f"  latent_norm: min={latent_norm.min():.4f}, max={latent_norm.max():.4f}, std={latent_norm.std():.4f}")
    print(f"  quantized:   min={quantized_test.min():.4f}, max={quantized_test.max():.4f}, std={quantized_test.std():.4f}")
    uniq = torch.unique(quantized_test.flatten()[:5000])
    print(f"  unique values (first 5000): {len(uniq)}")
    if len(uniq) > 1:
        step = (uniq.max() - uniq.min()) / (len(uniq) - 1)
        print(f"  approx step size: {step.item():.6f}")
QUANT_STEP = 2.0 / 255  # assuming 256 levels over [-1,1]
print(f"Using quantizer step = {QUANT_STEP:.6f}\n")

# ---------- Latent scaling (tunable) ----------
LATENT_SCALE = 1.0   # increase if bpp is still high after 10 epochs
print(f"Latent scale factor = {LATENT_SCALE}\n")

# ---------- Entropy model and quantizer (training mode) ----------
entropy_model = HyperpriorEntropy(latent_dim=192, hyper_channels=320).to(device)  # increase to 512 for better rate
quantizer_train = Quantizer(num_levels=256, mode='training').to(device)

# Resume if checkpoint exists
checkpoint_dir = '/root/le-maia/checkpoints_entropy_stable_final'
...
if os.path.exists(resume_ckpt):
    ...
    # Optionally resume epoch number from a separate file
...
# ---------- Discrete likelihood with step size ----------
...
for epoch in range(start_epoch, EPOCHS+1):
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        with torch.no_grad():
            ...
            # Scale latents to better use quantizer range
            latent_scaled = latent_norm * LATENT_SCALE
        
        quantized = quantizer_train(latent_scaled)
        ...
        # Diagnostic every 100 batches
        if batch_idx % 100 == 0 and batch_idx > 0:
            with torch.no_grad():
                residual = quantized - mu
                print(f"\n[Debug] batch={batch_idx}:")
                print(f"  mu: mean={mu.mean().item():.4f}, std={mu.std().item():.4f}")
                print(f"  residual: mean={residual.mean().item():.4f}, std={residual.std().item():.4f}, abs_mean={residual.abs().mean().item():.4f}")
                print(f"  sigma: mean={sigma.mean().item():.4f}, min={sigma.min().item():.4f}, max={sigma.max().item():.4f}")
