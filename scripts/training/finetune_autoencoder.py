...
# Load pre-trained NLL entropy model (non-zero bits)
...
print("Loaded NLL entropy model (frozen)")
...
print("Loaded affine autoencoder")
...
# Fine-tune autoencoder with rate loss from frozen entropy model
optimizer = optim.AdamW(autoencoder.parameters(), lr=1e-5, weight_decay=0.01)
...
LAMBDA = 0.1  # rate weight (higher = more compression)
EPOCHS = 50
...
for epoch in range(1, EPOCHS+1):
    ...
    total_dist = 0
    num_batches = 0
    pbar = tqdm(dataloader, desc=f"Fine-tune Epoch {epoch}/{EPOCHS}")
    for batch in pbar:
        ...
        # Rate from frozen entropy model (KL divergence)
        with torch.no_grad():
    ...
    if epoch % 10 == 0:
        torch.save(autoencoder.state_dict(), f'/root/le-maia/checkpoints_autoencoder_finetuned/epoch{epoch}.pt')

torch.save(autoencoder.state_dict(), '/root/le-maia/checkpoints_autoencoder_finetuned/final.pt')
