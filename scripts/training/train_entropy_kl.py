...
# KL divergence loss (what we want for rate)
def kl_loss(mu, sigma, x):
    var = sigma ** 2
    kl = 0.5 * (mu**2 + var - torch.log(var) - 1)
    return kl.sum() / x.numel()

optimizer = optim.AdamW(entropy_model.parameters(), lr=1e-4, weight_decay=0.01)
scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=100)

EPOCHS = 100
os.makedirs('/root/le-maia/checkpoints_entropy_kl', exist_ok=True)
...
for epoch in range(1, EPOCHS+1):
    ...
    for batch in pbar:
        ...
        loss = kl_loss(mu, sigma, quantized)
        optimizer.zero_grad()
        loss.backward()
        ...
        total_kl += loss.item()
        num_batches += 1
        pbar.set_postfix(kl=loss.item())
    scheduler.step()
    avg_kl = total_kl / num_batches
    print(f"Epoch {epoch}: Average KL = {avg_kl:.6f}")
    if epoch % 20 == 0:
        torch.save(entropy_model.state_dict(), f'/root/le-maia/checkpoints_entropy_kl/entropy_epoch{epoch}.pt')

torch.save(entropy_model.state_dict(), '/root/le-maia/checkpoints_entropy_kl/entropy_final.pt')
