...
# Optional resume from previous good checkpoint
...
# ---------- Stable NLL loss with safeguards ----------
def gaussian_nll(y, mu, log_sigma):
    sigma = torch.nn.functional.softplus(log_sigma) + 1e-4          # higher floor
    sigma = torch.clamp(sigma, min=1e-3, max=10.0)                  # prevent collapse
    nll = 0.5 * torch.log(2 * torch.pi * sigma**2) + 0.5 * ((y - mu) / sigma)**2
    return nll.mean()

optimizer = optim.AdamW(entropy_model.parameters(), lr=2e-4, weight_decay=1e-5)  # more stable
...
for epoch in range(1, EPOCHS + 1):
    ...
    avg_bpp = avg_rate / np.log(2)
    print(f"Epoch {epoch}: Avg NLL = {avg_rate:.6f}  →  {avg_bpp:.4f} bpp")
...
print("✅ Entropy model training complete (stable NLL version).")
