...
# Load affine autoencoder (frozen)
...
# Dataset that returns frames, then we compute latents on the fly
...
EPOCHS = 50
os.makedirs('/root/le-maia/checkpoints_entropy_autoencoder', exist_ok=True)
...
for epoch in range(1, EPOCHS+1):
    ...
    for batch in pbar:
        ...
        with torch.no_grad():
            ...
            latent_norm = autoencoder.encode(batch_flat)  # [B*T, 192, 16, 16]
        # Quantize
        quantized = quantizer(latent_norm)
        # Get mu, sigma from entropy model
    ...
    if epoch % 10 == 0:
        torch.save(entropy_model.state_dict(), f'/root/le-maia/checkpoints_entropy_autoencoder/epoch{epoch}.pt')

torch.save(entropy_model.state_dict(), '/root/le-maia/checkpoints_entropy_autoencoder/final.pt')
