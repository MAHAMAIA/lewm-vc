...
# Load pre-trained affine autoencoder weights if available
...
def nll_loss(mu, sigma, x):
    """Negative log-likelihood loss for quantized latents."""
    var = sigma ** 2
    loss = 0.5 * ((x - mu) ** 2 / var + torch.log(var) + np.log(2 * np.pi))
    return loss.sum() / x.numel()

# Rate-distortion tradeoff (start with low lambda, increase later)
LAMBDA = 0.1

EPOCHS = 100
os.makedirs('/root/le-maia/checkpoints_joint_nll', exist_ok=True)
...
for epoch in range(1, EPOCHS+1):
    ...
    for batch in pbar:
        ...
        # Get entropy model parameters
        params = entropy_model.hyperprior_cnn(quantized)
        mu = params[:, :192, :, :]
        log_sigma = params[:, 192:, :, :]
        sigma = torch.nn.functional.softplus(log_sigma) + 1e-5
        
        # Rate = negative log-likelihood (bits)
        nll = nll_loss(mu, sigma, quantized)
        rate_bits = nll * quantized.numel() * np.log2(np.e)  # convert to bits
    ...
    # Increase lambda after 50 epochs to force more compression
    if epoch == 50:
        LAMBDA = 1.0
        print(f"Increased LAMBDA to {LAMBDA}")
    
    if epoch % 20 == 0:
        torch.save({
            'autoencoder': autoencoder.state_dict(),
            'entropy_model': entropy_model.state_dict(),
        }, f'/root/le-maia/checkpoints_joint_nll/checkpoint_epoch{epoch}.pt')

torch.save(autoencoder.state_dict(), '/root/le-maia/checkpoints_joint_nll/autoencoder_final.pt')
torch.save(entropy_model.state_dict(), '/root/le-maia/checkpoints_joint_nll/entropy_final.pt')
