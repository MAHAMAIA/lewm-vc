...
Loss = λ·Rate + Distortion (MSE + perceptual)
"""
...
# ---------- Load trained entropy model (contextual) ----------
...
entropy_model.load_state_dict(torch.load(entropy_ckpt, map_location=device, weights_only=False))
entropy_model.eval()
...
# Fine‑tune with a small λ (start with 0.001, then sweep)
LAMBDA = 0.001
...
for epoch in range(1, EPOCHS+1):
    ...
    for batch in pbar:
        ...
        # Rate from frozen entropy model
        with torch.no_grad():
            ...
            rate_bits = nll * quantized.numel() / np.log(2)  # total bits
    ...
    if epoch % 10 == 0:
        torch.save(autoencoder.state_dict(), f'/root/le-maia/checkpoints_finetuned_rd/ae_lambda_{LAMBDA}_epoch{epoch}.pt')

torch.save(autoencoder.state_dict(), f'/root/le-maia/checkpoints_finetuned_rd/ae_lambda_{LAMBDA}_final.pt')
