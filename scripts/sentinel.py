#!/usr/bin/env python3
"""
sentinel — Feature compression CLI for LeWM-VC (Sentinel)

Usage:
  sentinel feature encode --model model.pt --input frame.png --output bitstream.bin
  sentinel feature decode --model model.pt --input bitstream.bin --output features.pt
  sentinel feature info --model model.pt

Commands:
  feature encode   Compress frames to feature bitstream
  feature decode   Decompress bitstream back to features
  feature info     Show model architecture and statistics
"""
...
def cmd_info(args):
    device = args.device
    _, info = load_model(args.model, device)
    ...
    total_params = (
        sum(
            p.numel()
            for m in [models["compressor"], models["decompressor"], models["entropy_model"]]
    ...
    # Load to count params
    ckpt = torch.load(args.model, map_location="cpu")
    comp_params = sum(
        p.numel()
        for p in FeatureCompressor(
            in_channels=info["feature_channels"],
            latent_dim=info["latent_dim"],
        ).parameters()
    )
    decomp_params = sum(
        p.numel()
        for p in FeatureDecompressor(
            latent_dim=info["latent_dim"],
            out_channels=info["feature_channels"],
        ).parameters()
    )
    entropy_params = sum(
        p.numel()
        for p in HyperpriorEntropy(
            latent_dim=info["latent_dim"],
            hyper_channels=32,
        ).parameters()
    )
